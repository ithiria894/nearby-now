-- Banner choice (#72 / WEB_PLAN §3.3). Nullable text:
--   'cat:<key>'   explicit category pick on /new
--   'img:<path>'  creator upload (phase 2)
--   null          platform-assigned (auto-detect from title → hash → LLM later)
-- Mobile adopts later. get_feed_public re-exposes it so cards can honor the
-- explicit choice over the derived category.

alter table public.activities add column if not exists banner text;

-- get_feed_public gains `banner`; drop+recreate (return type changes).
drop function if exists public.get_feed_public(text, text, double precision, double precision, double precision, int);

create function public.get_feed_public(
