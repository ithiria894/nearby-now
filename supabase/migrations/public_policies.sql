...
-- Generated at: 2026-01-25T08:49:32Z
-- ============================================================
...
-- Policy: members_insert_self on public.activity_members

CREATE POLICY members_insert_self ON public.activity_members AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((user_id = auth.uid()));
...
-- Policy: members_update_self on public.activity_members

CREATE POLICY members_update_self ON public.activity_members AS PERMISSIVE FOR UPDATE TO authenticated USING ((user_id = auth.uid())) WITH CHECK ((user_id = auth.uid()));
...
-- Policy: room_events_insert_members on public.room_events

CREATE POLICY room_events_insert_members ON public.room_events AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (((user_id = auth.uid()) AND (EXISTS ( SELECT 1
